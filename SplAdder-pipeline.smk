...
df_names = pd.DataFrame({
    ...
    'spladder_run': ["_".join(n.split("_")[:2]) for n in longNames],
...
ctrl_dict = df_names[['spladder_run', 'control']].drop_duplicates().set_index('spladder_run')['control'].to_dict()
spladder_run = df_names[~df_names['spladder_run'].str.contains(CONTROL)]['spladder_run'].unique().tolist()
print(bamlist)
print(spladder_run)
########## OUTPUTS ##########
...
rule all:
        input:
                ...
                expand(OUTDIR + "line_bam_lists/bam_{bamlist}.txt", bamlist=bamlist),
                expand(OUTDIR + "spladder_{spladder_run}.done", spladder_run=spladder_run)
...
rule list_bams:
        input:
                ...
                expand(OUTDIR + "alignment/{sample}.bam.bai",sample=SAMPLES),
        output:
                bl = OUTDIR + "line_bam_lists/bam_{bamlist}.txt"
        params:
                bn = "{bamlist}",
                alignment_dir = OUTDIR + "alignment",
                bam_lists_dir = OUTDIR + "line_bam_lists",
                tool_type = TOOL_TYPE
        shell:
                "scripts/list_bams.sh {params.bn} {params.alignment_dir} {params.bam_lists_dir} {params.tool_type}"

def get_ctrl(wildcards):
    spladder_run = wildcards.spladder_run
    return OUTDIR+"line_bam_lists/bam_"+ctrl_dict[spladder_run]+".txt"


rule spladder_build:
        input:
                b2=OUTDIR+"line_bam_lists/bam_{spladder_run}.txt",
                b1=get_ctrl,
        output:
                touch(OUTDIR+"spladder_{spladder_run}.done")
        params:
                od=OUTDIR + "spladder_{spladder_run}/",
                gtf=GTF,
                bc=OUTDIR+"line_bam_lists/bam_{spladder_run}_combined.txt"
        conda:
                "spladder"
        shell:
                """
                cat {input.b1} {input.b2} > {params.bc} &&
                spladder \
                build \
                --set-mm-tag nM \
                --bams {params.bc} \
